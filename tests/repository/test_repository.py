...
class RepositoryTests:
    ...
    def setup_method(
        self, aggregate_root_cls_mock, snapshot_mock, repository_backend_mock, importer_mock
    ):
        self.aggregate_root_cls = aggregate_root_cls_mock()
        self.snapshot = snapshot_mock(return_value=None)
        self.repository_backend = repository_backend_mock(return_value=self.aggregate_events)
        ...
        self.repository = partial(
            Repository,
            config=self.config,
            aggregate_root_cls=self.aggregate_root_cls,
    ...
    def test_should_apply_events_when_loading_from_event_storage(self, aggregate_root_cls_mock):
    ...
    def test_repository_commit_should_call_backend_and_snapshot(self, aggregate_root_cls_mock):
        aggregate_root_cls = aggregate_root_cls_mock(exhaust_events=False)
